            ...
            resp = get_with_retry(
                ...
                params={
                    ...
                    "fields":   "id,first_name,last_name,preferred_name,nickname,"
                                "grade,gender,status,updated_at,parent_ids,date_of_birth",
                },
            )
            data  = resp.json()
...
def sc_get_all_students(token: str) -> dict[str, dict]:
    ...
    while True:
        ...
        resp = requests.get(url, headers=sc_headers(token), timeout=30)
        if not resp.ok:
            log.error(f"SC Students error: {resp.status_code} — {resp.text}")
        resp.raise_for_status()
        data = resp.json()
...
def sc_get_all_contacts(token: str) -> dict[str, dict]:
    ...
    while True:
        ...
        resp = requests.get(url, headers=sc_headers(token), timeout=30)
        if not resp.ok:
            log.error(f"SC Contacts error: {resp.status_code} — {resp.text}")
        resp.raise_for_status()
        data = resp.json()
...
def sc_create_student(token: str, oa_student: dict) -> dict:
    payload = {
        ...
        "dateOfBirth": oa_student.get("date_of_birth"),
    }
