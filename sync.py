...
def run_sync():
    ...
    for oa_s in oa_staff_list:
        oa_id_str = str(oa_s["id"])
        is_deleted = bool(oa_s.get("deleted_at") or oa_s.get("archived_at"))

        try:
            existing_sc = sc_staff.get(oa_id_str)

            # ── ARCHIVE: deleted in OA ────────────────────────────────────────
            if is_deleted:
                if existing_sc and not existing_sc.get("isArchived"):
                    log.info(f"Archiving staff OA:{oa_id_str} ({oa_s.get('last_name')}, {oa_s.get('first_name')})")
                    sc_archive_staff(sc_token, existing_sc["id"], archive=True)
