...
def run_sync():
    ...
    stats = {"created": 0, "updated": 0, "archived": 0,
             ...
             "staff_created": 0, "staff_updated": 0, "staff_archived": 0,
             "errors": 0}
    ...
    # Build a set of active OA staff IDs for quick lookup
    oa_active_staff_ids = {str(s["id"]) for s in oa_staff_list}

    log.info("Fetching existing staff from SC...")
    sc_staff = sc_get_all_staff(sc_token)

    # ── Step A: Archive SC staff who no longer appear in OA ──────────────────
    for sc_id_str, sc_s in sc_staff.items():
        if sc_id_str not in oa_active_staff_ids and not sc_s.get("isArchived"):
            log.info(f"Archiving staff OA:{sc_id_str} ({sc_s.get('lastName')}, {sc_s.get('firstName')}) — no longer in OA")
            try:
                sc_archive_staff(sc_token, sc_s["id"], archive=True)
                stats["staff_archived"] += 1
            except Exception as e:
                log.error(f"Error archiving staff OA:{sc_id_str}: {e}")
                stats["errors"] += 1
    ...
    log.info(f"Staff     — created: {stats['staff_created']}, updated: {stats['staff_updated']}, archived: {stats['staff_archived']}")
    if stats["errors"]:
