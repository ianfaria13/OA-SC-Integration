    ...
    for oa_s in oa_students:
        ...
        try:
            ...
            # ── UPDATE: enrolled, already in SC ──────────────────────────────
                ...
                log.info(f"Comparing student OA:{oa_id_str} — "
                         f"OA grade='{oa_s.get('grade')}' SC grade='{existing_sc.get('grade')}' | "
                         f"OA gender='{oa_s.get('gender')}' SC gender='{existing_sc.get('gender')}' | "
                         f"OA firstName='{oa_s.get('first_name')}' SC firstName='{existing_sc.get('firstName')}'")
                if needs_student_update(oa_s, existing_sc):
                    ...
                    log.info(f"No changes for student OA:{oa_id_str}")
